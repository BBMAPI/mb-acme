...
checktls(){
if [[ -f /root/mbca/cert.crt && -f /root/mbca/private.key ]] && [[ -s /root/mbca/cert.crt && -s /root/mbca/private.key ]]; then
cronac
green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/mbca文件夹内" 
yellow "公钥文件crt路径如下，可直接复制"
green "/root/mbca/cert.crt"
yellow "密钥文件key路径如下，可直接复制"
green "/root/mbca/private.key"
ym=`bash ~/.acme.sh/acme.sh --list | tail -1 | awk '{print $1}'`
echo $ym > /root/mbca/ca.log
if [[ -f '/etc/hysteria/config.json' ]]; then
blue "检测到Hysteria-1代理协议，如果你安装了mb的Hysteria脚本，请在Hysteria脚本执行申请/变更证书，此证书将自动应用"
fi
if [[ -f '/etc/caddy/Caddyfile' ]]; then
blue "检测到Naiveproxy代理协议，如果你安装了mb的Naiveproxy脚本，请在Naiveproxy脚本执行申请/变更证书，此证书将自动应用"
fi
if [[ -f '/etc/tuic/tuic.json' ]]; then
blue "检测到Tuic代理协议，如果你安装了mb的Tuic脚本，请在Tuic脚本执行申请/变更证书，此证书将自动应用"
fi
if [[ -f '/usr/bin/x-ui' ]]; then
blue "检测到x-ui（xray代理协议），如果你安装了mb的x-ui脚本，开启tls选项，此证书将自动应用"
fi
if [[ -f '/etc/s-box/sb.json' ]]; then
blue "检测到Sing-box内核代理，如果你安装了mb的Sing-box脚本，请在Sing-box脚本执行申请/变更证书，此证书将自动应用"
fi
else
bash ~/.acme.sh/acme.sh --uninstall >/dev/null 2>&1
rm -rf /root/mbca
...
green "例：原二级域名 x.mb.eu.org 或 x.mb.cf ，在cloudflare中重命名其中的x名称"
echo
...
installCA(){
bash ~/.acme.sh/acme.sh --install-cert -d ${ym} --key-file /root/mbca/private.key --fullchain-file /root/mbca/cert.crt --ecc
}
...
yellow "WARP未能自动关闭，请手动关闭！或者使用支持自动关闭与开启的mbWARP脚本"
else
...
if [[ -n $caacme1 && ! $caacme1 == "Main_Domain" ]] && [[ -f /root/mbca/cert.crt && -f /root/mbca/private.key && -s /root/mbca/cert.crt && -s /root/mbca/private.key ]]; then
caacme=$caacme1
else
caacme='无证书申请记录'
fi
